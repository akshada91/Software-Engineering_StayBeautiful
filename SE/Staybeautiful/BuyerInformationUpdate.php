<?php
	...
	<h1 style="text-align: center">Edit Buyer Information</h1>
	...
	if (isset($_GET['ID']))
	{
		...
		if (isset($_POST['submit']))
		{
			...
			if (is_numeric($_POST['ID']))
			{
				...
				else
				{
					...
					echo $Zipcode;
					echo $Phone;
					if ($stmt = $mysqli->prepare("UPDATE address SET Street1 = ?, Street2 = ?, City = ?, State = ?, Zipcode = ?, Phone = ?
					WHERE ID= ?"))
					{
		...
		else
		{
			...
			if (is_numeric($_GET['ID']) && $_GET['ID'] > 0)
			{
				...
				if($stmt = $mysqli->prepare("SELECT * FROM address WHERE ID=?"))
				{
